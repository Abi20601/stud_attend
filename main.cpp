...
const double CHANGE_IN_SECONDS_FOR_COLLECTION = 2.0;       // How much time must pass before collecting a new face photo for training.
const char *windowName = "Webcam FaceRec for Attendance";   // Name shown in the GUI window.
...
// Declare a Map Structure to Save the Entries 
struct studInfo{
    int Identity;
    string sName;
};

map<int,studInfo> studentData;
...
string inputChoice = "No";

// Checks if Student is already present for the current date

int checkIfUpdated(string stud_Name)
{
 string chkQry = "SELECT Date(Date) from info_attend where Student_Name='";
	chkQry += stud_Name;
	chkQry += "' and Date(Date) = CURDATE()";
  
 cout << "\n Query to Database is :" << chkQry << endl;
 
        query_state=mysql_query(conn, chkQry.c_str());
	...
	if(query_state == 1)
	   ...
	   {
     		 cout<< "\n Entry for stud_Name is present for :"<< row[0] << endl;
	         return 1;	 
	   }
	 } 
...
int updateTable(string sName, string studSts)
{
   const char *server="localhost";
   const char *user="root";
   const char *password="admin";
   const char *database="dbAttendance";
   int retVal = -127;
   query_state = 0;
   ...
   if(conn==NULL)
   {
       cout << "\n Error Connecting to MYSQL database :" << database << endl;	 
	...
	{
		cout << "\n Successfully Connected to Database" << endl;
	}
   // Check if Attendance for the day for the entry is updated already or not
   retVal =  checkIfUpdated(sName);
  
    
   if (retVal == 1 )
	{
		cout << "\n Result is already Updated for Today, Skipping Duplication."<< endl;
	}

   else if( retVal == -1)
	{

		cout << "\n Failed to insert attendance in  database as the Database Connection is not done" << endl;
	}
   else
	{

	   string execQry="INSERT INTO  info_attend(Student_Name,Status) VALUES ('";
		  execQry += sName;
	  	  execQry +="','";
	  	  execQry += studSts;
	  	  execQry +="')";

  	 	cout << "\n Query to Database is :" << execQry << endl;
  	        query_state=mysql_query(conn, execQry.c_str());
		   if(query_state!=0)
  		 {
	   ...
	   cout<<"\n Entry Written in database."<<endl<<endl;
        }
...
void onMouse(int event, int x, int y, int, void*)
{
    ...
    Point pt = Point(x,y);
    if (isPointInRect(pt, m_rcBtnAdd) || inputChoice == "Yes") {
        ...
        if ((m_numPersons == 0) || (m_latestFaces[m_numPersons-1] >= 0)) {
            ...
            if(inputChoice!="Yes") 
           {
           // Get Here the name and information from the User
	     mapSize = studentData.size();
             string fName("");
	     string mName("");
	     string lName("");
	     ...
	     cin >> lName;
	     infoStudent.sName = fName + " " +  mName + " " +lName;
	     infoStudent.Identity = mapSize;
	     curFileName = fName + "_" + mName + "_" + lName; 
            // Insert the User identity in map structure
	     studentData.insert(pair<int,struct studInfo>(mapSize,infoStudent));	 
             cout << "\n Length of Map structure is now : " << studentData.size() << endl;	
	  }
        }
...
int countEntriesInDir(const char* dirname,vector<string> &files)
{
        int n = 0 ;
        dirent* dirp;
         ...
         if((string(dirp->d_name)==".") || (string(dirp->d_name)== ".."))
	   {
        ...
        closedir(dir);
	cout << "\n Directoty has :" << n << "Images." << endl;
        return n;
}

string execCmd(string inCmd)
{
        string outRes("");
        FILE *pin = popen(inCmd.c_str(),"r");
        if(!pin) return "ERROR";
...
void recognizeAndTrainUsingWebcam(VideoCapture &videoCapture, CascadeClassifier &faceCascade, CascadeClassifier &eyeCascade1, CascadeClassifier &eyeCascade2 , string inputChoice ="No")
{
    Ptr<FaceRecognizer> model;
    vector<Mat> preprocessedFaces;
    vector<Mat> oldprocessedFaces;
    ...
    // Start in Detection mode only if user doesnot want to start Recognition
   if(inputChoice =="Yes")	
	{ 
  	 // m_mode = MODE_RECOGNITION;
	 int numEntries = 0;
	// Load Previous saved images at startup
	     numEntries =  countEntriesInDir("../studentImages",files);
	      for (int j=0; j< numEntries ; j++)
	       {
	          string fName("../studentImages/");
                   	 fName += files[j];
			 //fName +=".bmp";
			 cout << "Fetching Image : "<< fName << endl;		
			 oldprocessedFaces.push_back(imread(fName));
		}
        } 
	...
	bool gotFaceAndEyes = false;
        if (preprocessedFace.data || preprocessedFaces.size())
	{
            gotFaceAndEyes = true;
	}
        ...
        if (m_mode == MODE_DETECTION) {
            ...
            if (gotFaceAndEyes) {
                ...
                if ((imageDiff > CHANGE_IN_IMAGE_FOR_COLLECTION) && (timeDiff_seconds > CHANGE_IN_SECONDS_FOR_COLLECTION)) {
                    ...
                    if(inputChoice == "Yes")
		    { 
		//Added here /home/pi/modifiedfcr
                    	string testImage ("/home/pi/modifiedfcr/testImage/testSample.bmp");
		    	imwrite(testImage.c_str(),preprocessedFace);	 
		    }

		    if(inputChoice !="Yes")
		    {
		//Added here /home/pi/modifiedfcr
		    string fileName("/home/pi/modifiedfcr/studentImages/");
		           //fileName += toString(m_selectedPerson);
		           fileName += curFileName; 
			   fileName +=".bmp";
                    imwrite(fileName.c_str(),preprocessedFace);  
		    ...
		    if( preprocessedFaces.size() && inputChoice == "Yes")
			m_mode = MODE_RECOGNITION;
		    else
			m_mode = MODE_DETECTION;
                }
	   ...
	   if(inputChoice == "No") // change these two lines
           {
	...
	if (haveEnoughData)  {
            ...
            {
              m_mode = MODE_RECOGNITION; // Added
            }	 

      }
        else if (m_mode == MODE_RECOGNITION) {

             if (inputChoice == "Yes" && preprocessedFaces.size())
		{
		  for(int k = 0 ; k < oldprocessedFaces.size();k++)
	      	  {
		   string inCmd("python ../compareImg.py ../studentImages/");
		   inCmd += files[k];
		   inCmd += " ../testImage/testSample.bmp"; 
		   string removeFile = "rm -f ../testImage/testSample.bmp"; 
		//Added string to the above line
		   cout << "\n Command is : " << inCmd << endl;
		   double similarity =  atof((execCmd(inCmd.c_str())).c_str());
                if(similarity > 50.00) 
		{
		  //simlarity>60
		  cout<<"Inside similarity>50";
                  cout <<"Images Matched with similarity :" << similarity << endl;
		  string rawName; 
		  size_t lastindex = files[k].find_last_of("."); 
	 	  rawName = files[k].substr(0, lastindex);
		  replace(rawName.begin(),rawName.end(),'_', ' '); 
		 // cout << "\n Student Name is : "  << studentName << endl; 
		  cout << "Student name is : " << rawName << endl; 
	     	  cout << "\n Updating Database: " << endl;
	     	  updateTable(rawName ,"Present");
		  cout << "\n Removing the detected entry from the map list "  << endl;
		  cout << "\n Removing Newly Clicked Image" << endl;
		  execCmd(removeFile.c_str());
		  files.erase(files.begin() + k); 
		  oldprocessedFaces.erase(oldprocessedFaces.begin() + k);
                  m_mode = MODE_DETECTION;
                  break;
                }
              } 
            ...
            if (gotFaceAndEyes && (preprocessedFaces.size() > 0) && (preprocessedFaces.size() == faceLabels.size())) {
               ...
               double similarity = getSimilarity(preprocessedFace, reconstructedFace);
                ...
                cout << "\n Identity: " << outputStr << ". Similarity: " << similarity << endl;
	       
                if(similarity > 0.500) 
		{ 
                /* iterate thru map and verify */
    		map<int,struct studInfo>::iterator it = studentData.begin();
  		  while(it != studentData.end())
			 {
       				if(it->first == identity)
					{
					 cout << " \n Recognized Face is of Student : " << it->second.sName << endl; 
	     				 cout << "\n Updating Database: " << endl;
	     				 updateTable(it->second.sName ,"Present");
					}
        				it++;
    			}
                }		
...
int main(int argc, char *argv[])
{
    ...
    cout << "Do u Want to Start Recognition Directly (Yes/No) :"<< endl;
    cin >> inputChoice; 	
     
    setMouseCallback(windowName, onMouse, 0);

    // Run Face Recogintion interactively from the webcam. This function runs until the user quits.
    recognizeAndTrainUsingWebcam(videoCapture, faceCascade, eyeCascade1, eyeCascade2,inputChoice);
